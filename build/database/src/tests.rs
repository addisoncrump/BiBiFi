...
fn basic_full_3() -> Result<(), Box<dyn Error>> {
    ...
    my_database.delegate(&"admin".to_string(), &Target::Variable("my_var3".to_string()), &"alice".to_string(),
                        &Right::Read, &"git".to_string());

    // delete permissions to git
    my_database.undelegate(&"admin".to_string(), &Target::Variable("my_var3".to_string()), &"alice".to_string(),
                         &Right::Read, &"git".to_string());

    assert_eq!(my_database.check_right(&"my_var3".to_string(), &Right::Read, &"git".to_string()), false);
