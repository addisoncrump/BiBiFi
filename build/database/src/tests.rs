...
fn basic_full_1() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(my_database.check_pass(&"tom".to_string(), &hash("tom".to_string())), &hash("".to_string())), DENIED);

    // lets say, bob created my_var and delegated all permissions to everyone
    my_database.set(&"bob".to_string(), &"my_var".to_string(), &Value::Immediate("lmao".to_string()));
    ...
    my_database.delegate(
        &Target::Variable("my_var2".to_string()),
        &"admin".to_string(),
        &Right::Read,
        &"alice".to_string(),
    );
